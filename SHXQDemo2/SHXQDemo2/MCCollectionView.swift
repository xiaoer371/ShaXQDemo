...
protocol MCCollectionViewDelegate :NSObjectProtocol {
    
    func deleteWihthItemModel(model:Contact)
    func didSelectItem(model:Contact)
    func addDataSourceItem()
    func deleteDataSourceItem()

}

let   xCellRowNumber :CGFloat = 5.0;


class MCCollectionView: UIView {
    var delegate :MCCollectionViewDelegate?
